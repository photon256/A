...
bot = Client("bot",    
   bot_token="7439309084:AAF4OQUUiPL6eAI556ayybwEXgBmOR5GukY",    
   api_id= 21179966,    
...
async def account_login(bot: Client, m: Message):    
    ...
    try:    
        for i in range(count - 1, len(links)):
            ...
            elif "apps-s3-jw-prod.utkarshapp.com" & 'enc_plain_mp4' & '.mp4' in url:
                url = url.replace(url.split("/")[-1], res+'.mp4')
                
            elif 'apps-s3-jw-prod.utkarshapp.com' & 'Key-Pair-Id' & '.mp4?' in url:
                url = None
                    
            elif 'apps-s3-jw-prod.utkarshapp.com' & '.m3u8' in url:
